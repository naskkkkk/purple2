...
module.exports = {
    name: "líder",
    alias: ["rankd", "ranking", "top"],
    description: "sim",
    timeout: 1000,
  async execute (client, message, args){

    const embed = new Discord.MessageEmbed()
    .setDescription(`**TOP 10 RANKS GLOBAL <:736052240669999135:817942453122105344> <a:Diamante:814778058556309534> **\n\n**a!Rank Ienes\na!Rank Waifus\na!Rank Animes\na!Rank Mangá**`)
  ...
  if(!args[0]) return message.channel.send(embed) 
    ...
    if (args[0] == 'ienes') {
             ...
             let money = db.all().filter(data => data.ID.startsWith(`ienes_`)).sort((a, b) => b.data - a.data);
        if (!money.length) {
      ...
      let bal = db.fetch(`ienes_${message.guild.id}_${message.author.id}`)
       if(bal === null) bal = '0'
        const embed = new MessageEmbed()
            .setTitle(`**<:vipouro2:814807164366749747> <:736052240669999135:817942453122105344> __*GLOBAL* | TOP.10 IENES __**`)
            .setColor("#ff58c3")
            .setDescription(finalLb)
            .setFooter(`Seus Ienes » ${bal}¥ | Trabalhe mais e suba de Rank!`, bot.user.displayAvatarURL())
            .setTimestamp()
        message.channel.send(embed);

  } else if(args[0] == 'waifus') { 
 
             let waifu = db.all().filter(data => data.ID.startsWith(`waifus_`)).sort((a, b) => b.data - a.data);
        if (!waifu.length) {
            let noEmbed = new MessageEmbed()
                .setAuthor(message.member.displayName, message.author.displayAvatarURL())
                .setColor("#ff58c3")
        ...
        const embed = new MessageEmbed()
            .setTitle(`**<:vipouro2:814807164366749747> <:736052240669999135:817942453122105344> __*GLOBAL* | TOP.10 WAIFUS__**`)
            .setColor("#ff58c3")
        ...
        const embed = new MessageEmbed()
            .setTitle(`**<:vipouro2:814807164366749747> <:736052240669999135:817942453122105344> __*GLOBAL* | TOP.10 ANIMES__**`)
            .setColor("#ff58c3")
            .setDescription(pikaLb)
            .setFooter(bot.user.tag, bot.user.displayAvatarURL())
            .setTimestamp()
        message.channel.send(embed);
  } else if(args[0] == 'mangás') {
                 let mang = db.all().filter(data => data.ID.startsWith(`mangas_`)).sort((a, b) => b.data - a.data);
        if (!mang.length) {
            let noEmbed = new MessageEmbed()
                .setAuthor(message.member.displayName, message.author.displayAvatarURL())
                .setColor("#ff58c3")
        ...
        const embed = new MessageEmbed()
            .setTitle(`**<:vipouro2:814807164366749747> <:736052240669999135:817942453122105344> __*GLOBAL* | TOP.10 MANGÁS__**`)
            .setColor("#ff58c3")
